...
export type AgentDraftStatus = 'DRAFT' | 'GENERATED'
...
export type AgentDraft = { id: number; owner_id: number; name: string; description: string | null; framework: AgentFramework; mcp_id: number; mcp_version_id: number; mcp_version: number; mcp_connections: AgentMcpConnection[]; system_prompt: string; user_prompt: string; llm_model_id: number | null; llm_provider: string | null; llm_model_name: string | null; temperature: number | null; max_output_tokens: number | null; project_directory: string; project_revision: number; default_build_id: number | null; files: Record<string, string>; status: AgentDraftStatus; created_at: string; updated_at: string }
...
export type AgentBuild = { id: number; agent_id: number; owner_id: number; status: AgentBuildStatus; image_ref: string | null; logs: string; error: string | null; created_at: string; updated_at: string }
...
export type AgentDraftInput = Omit<AgentDraft, 'id' | 'owner_id' | 'files' | 'status' | 'project_revision' | 'default_build_id' | 'created_at' | 'updated_at'>
...
export function runAgentBuild(token: string, agentId: number, buildId: number, prompt: string) { return serviceRequest<AgentInvocationResponse>(agentApiBaseUrl, `/agents/${agentId}/builds/${buildId}/run`, token, { method: 'POST', body: JSON.stringify({ prompt }) }) }
...
export function sendAgentMessage(token: string, agentId: number, conversationId: number, prompt: string, buildId: number) { return serviceRequest<AgentMessage[]>(agentApiBaseUrl, `/agents/${agentId}/conversations/${conversationId}/messages`, token, { method: 'POST', body: JSON.stringify({ prompt, build_id: buildId }) }) }
